...
const source = readFileSync(
  "src/actions/role-responsibility-assessment.actions.ts",
  "utf8",
);
...
test("Ticket 25K.2 §3/§4: creation returns the durable assessmentId on success, not just success:true, so the caller can redirect without a second lookup", () => {
  assert.match(
    source,
    /export type CreateRoleResponsibilityAssessmentActionResult =\s*\n\s*\| \{ success: true; assessmentId: string \}/,
  );
  assert.match(
    source,
...
test("createRoleResponsibilityAssessmentAction's declared return type matches its actual result type", () => {
  assert.match(
    source,
    /export async function createRoleResponsibilityAssessmentAction\(\s*\n\s*values: unknown,\s*\n\s*\): Promise<CreateRoleResponsibilityAssessmentActionResult>/,
  );
});
