...
test("Ticket 25K.2 §28/§29: canEdit requires both DRAFT status and evaluator ownership — never granted to a different authorized manager, mirroring 25I's own evaluatorUserId check", () => {
  assert.match(
    source,
...
test("Ticket 25K.2 §64: opening this page performs no mutation — only a read (getRoleResponsibilityAssessmentDetail), no create/assess/submit/delete action import", () => {
  assert.match(source, /getRoleResponsibilityAssessmentDetail\(/);
  assert.doesNotMatch(
    source,
    /createRoleResponsibilityAssessmentAction|assessRoleResponsibilityItemAction|submitRoleResponsibilityAssessmentAction|deleteRoleResponsibilityAssessmentAction/,
  );
});

test("Ticket 25K.2 §65: this is the exact canonical route 25K.1's dashboard Continuer l’évaluation/Voir le détail CTA already deep-links to — /admin/performance-assessments/{assessmentId}, not a page-specific alternate", () => {
